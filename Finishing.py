...
def generatePrompt(datas, tower):
    body = {
      ...
      Read all data from this table carefully:
        ...
        need only json not explanation nad not string literal or any other
        json fromat
        [{{
...
def GetTower4Finishing(exceldatas):
    ...
    try:
        ...
        json_data = json.loads(Ai_answer)
            # st.write(Ai_answer)
        st.session_state.tower4_finishing = json_data[0]['percentage']
    except:
...
def GetTower5Finishing(exceldatas):
    ...
    try:
        ...
        Ai_answer = generatePrompt(datas.head()[['Activity Name', '% Complete']], "Tower 5")
        st.write(Ai_answer)
        json_data = json.loads(Ai_answer)
        st.session_state.tower5_finishing = json_data[0]['percentage']
    except:
...
def GetTowerGFinishing(exceldatas):
    ...
    try:
        ...
        st.write(Ai_answer)
        json_data = json.loads(Ai_answer)
        st.session_state.towerg_finishing = json_data[0]['percentage']
    except:
...
def GetTowerHFinishing(exceldatas):
    ...
    try:
        ...
        st.write(datas.head()[['Activity Name', '% Complete']])
        Ai_answer = generatePrompt(datas.head()[['Activity Name', '% Complete']], "Tower H")
        st.write(Ai_answer)
        json_data = json.loads(Ai_answer)
        st.session_state.towerh_finishing = json_data[0]['percentage']
    except:
