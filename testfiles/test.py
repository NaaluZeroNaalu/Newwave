...
#         lift_data = st.session_state.veridialift  # Fixed: was veridia_lift
#         common_area_data = st.session_state.veridiacommonarea  # Fixed: was veridia_Common_Area_Finishing
...
#     # Validate that all data are DataFrames and have required columns
#     main_datasets = [
...
#     # Safely get COS data with validation
#     cos_datasets = [
...
#     foundverdia = False
#     ignore_year = st.sidebar.number_input("Ignore Year", min_value=1900, max_value=2100, value=2023, step=1, key="ignore_year1")
#     ignore_month = st.sidebar.number_input("Ignore Month", min_value=1, max_value=12, value=3, step=1, key="ignore_month1")
...
#                     if ignore_month and ignore_year:
#                         st.session_state.slabreport = ProcessVeridia(io.BytesIO(response['Body'].read()), ignore_year, ignore_month)
#                     # st.write(veridia)
#                     foundverdia = True
#                     #  = veridia
#                     break
...
#                 except Exception as e:
#                     st.error(f"Failed to fetch file list from COS: {str(e)}")
#                     st.session_state['file_list'] = []
...
#         totals = getTotal(st.session_state.ai_response)
#         st.write("**AI Data Totals (Debug)**:")
#         st.write(totals)
...
#         if excel_file:
#             timestamp = pd.Timestamp.now(tz='India').strftime('%Y%m%d_%H%M')
#             file_name = f"Consolidated_Checklist_Veridia_{timestamp}.xlsx"
...
#     # Check if required session state keys exist
#     if not all(key in st.session_state for key in ['cos_client', 'bucket_name', 'file_list']):
#         st.error("COS client or file list not initialized. Please run 'Initialize and Fetch Data' first.")
#         st.session_state['slab_df'] = pd.DataFrame()
#     else:
#         try:
#             # Get the file_list from session state
#             cos_client = st.session_state['cos_client']
#             bucket_name = st.session_state['bucket_name']
#             file_list = st.session_state['file_list']  # This line was missing or commented out
            
#             GetSlabReport()  # Make sure this function is defined and works properly
            
#             # Now file_list is properly defined
#             struct_files = [file for file in file_list if 'Anti. Slab Cycle' in file['Key']]
#             st.write(f"**COS Slab Cycle Files Found**: {len(struct_files)}")
#             logging.info(f"Slab cycle files found: {struct_files}")
            
#             # Make sure slab_df is defined (either from GetSlabReport() or session state)
#             if 'slabreport' in st.session_state:
#                 slab_df = st.session_state['slabreport']
#             else:
#                 slab_df = pd.DataFrame()  # Fallback to empty DataFrame
                
#             st.write("**COS Slab Cycle DataFrame (Debug) - Green Cell Counts**:")
#             st.dataframe(slab_df)
#             logging.info(f"Slab cycle DataFrame fetched: {slab_df.to_dict() if not slab_df.empty else 'Empty DataFrame'}")

#             if slab_df.empty:
#                 st.warning("Slab cycle DataFrame is empty. No green cells found or file is invalid.")
                
#             st.session_state['slab_df'] = slab_df
#             logging.info(f"Stored slab_df in session state")
