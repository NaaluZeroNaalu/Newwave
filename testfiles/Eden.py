...
def generate_ncr_report(df, report_type, start_date=None, end_date=None):
    with st.spinner(f"Generating {report_type} NCR Report..."):
        ...
        for record in processed_data:
            ...
            # Updated logic to remove "Veridia" and align with new tower naming
            if any(phrase in description for phrase in ["clubhouse", "club"]):
                cleaned_record["Tower"] = "Club"
                logger.debug(f"Matched 'Clubhouse' in description: {description}")
            else:
                ...
                if multiple_tower_pattern:
                    cleaned_record["Tower"] = "Common Area"
                    ...
                    if tower_match:
                        ...
                        if "pour 1" in description:
                            cleaned_record["Tower"] = f"Tower {tower_num} Pour 1"
                        elif "pour 2" in description:
                            cleaned_record["Tower"] = f"Tower {tower_num} Pour 2"
                        else:
                            cleaned_record["Tower"] = f"Tower {tower_num} Common Description"
                        logger.debug(f"Single tower match: {tower_match.group(0)}, assigned: {cleaned_record['Tower']}")
                    elif "external development" in description:
                        cleaned_record["Tower"] = "External Development"
                        logger.debug(f"Matched 'External Development' in description: {description}")
                    else:
                        cleaned_record["Tower"] = "Common Area"
                        logger.debug(f"No specific tower match in description: {description}")
        ...
        for i in range(0, len(cleaned_data), chunk_size):
            ...
            prompt = (
                ...
                "Use 'Tower' values (e.g., 'Tower 4 Pour 1', 'Common Area') and 'Discipline_Category' values (e.g., 'SW', 'FW', 'MEP') exactly as provided. Count each record exactly once.\n\n"
    ...
    with st.spinner(f"Generating {report_type} Housekeeping NCR Report with WatsonX..."):
        ...
        for i in range(0, len(cleaned_data), chunk_size):
            ...
            prompt = (
                ...
                "Use the 'Tower' values exactly as they appear in the data (e.g., 'Club', 'Tower01', 'Common_Area'). "
...
def generate_consolidated_ncr_OpenClose_excel(combined_result, report_title="NCR"):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        indented_site_format = workbook.add_format({
            'align': 'left',
            'valign': 'vcenter',
            'border': 1,
            'indent': 1
        ...
        date_part = report_title.replace("NCR: ", "") if report_title.startswith("NCR: ") else "29_May"  # Current date: May 29, 2025
        
        # Define the hierarchical structure of sites
        towers = {
            "Tower 4": ["Pour 1", "Pour 2", "Common Description"],
            "Tower 5": ["Pour 1", "Pour 2", "Common Description"],
            "Tower 6": ["Pour 1", "Pour 2", "Common Description"],
            "Tower 7": ["Pour 1", "Pour 2", "Common Description"]
        }

        # Flat list for mapping purposes
        standard_sites = [f"{tower} {subcat}" for tower, subcats in towers.items() for subcat in subcats]

        def normalize_site_name(site):
            """
            Returns a tuple of (tower, subcategory) for the given site name.
            """
            if site in standard_sites:
                for tower, subcats in towers.items():
                    for subcat in subcats:
                        if site == f"{tower} {subcat}":
                            return "tower", tower, subcat
            match = re.search(r'(?:tower|t)[- ]?(\d+)', site, re.IGNORECASE)
            if match:
                num = match.group(1)
                tower = f"Tower {num}"
                if "pour 1" in site.lower():
                    return "tower", tower, "Pour 1"
                elif "pour 2" in site.lower():
                    return "tower", tower, "Pour 2"
                else:
                    return "tower", tower, "Common Description"
            return "unknown", None, site  # Fallback if site doesn't match expected format
        ...
        worksheet = workbook.add_worksheet(f'NCR Report {date_part}')
        ...
        row = 3
        all_towers_totals_resolved = {cat: 0 for cat in category_map.values()}
        all_towers_totals_open = {cat: 0 for cat in category_map.values()}
        all_towers_total = 0
        
        for tower, subcats in towers.items():
            # Calculate totals for this tower
            tower_totals_resolved = {cat: 0 for cat in category_map.values()}
            tower_totals_open = {cat: 0 for cat in category_map.values()}
            tower_total = 0
            
            # Aggregate totals from subcategories
            for subcat in subcats:
                site_key = f"{tower} {subcat}"
                original_resolved_key = next((k for k, v in site_mapping.items() if v == ("tower", tower, subcat)), None)
                original_open_key = next((k for k, v in site_mapping.items() if v == ("tower", tower, subcat)), None)
                site_total = 0
                
                # Resolved data
                for display_cat, json_cat in category_map.items():
                    value = 0
                    ...
                    tower_totals_resolved[json_cat] += value
                    site_total += value
                ...
                for display_cat, json_cat in category_map.items():
                    ...
                    tower_totals_open[json_cat] += value
                    site_total += value
                
                tower_total += site_total
            
            # Write the tower name and its totals
            worksheet.write(row, 0, tower, site_format)
            for i, (display_cat, json_cat) in enumerate(category_map.items()):
                worksheet.write(row, i+1, tower_totals_resolved[json_cat], cell_format)
            for i, (display_cat, json_cat) in enumerate(category_map.items()):
                worksheet.write(row, i+4, tower_totals_open[json_cat], cell_format)
            worksheet.write(row, 7, tower_total, cell_format)
            
            # Update all towers totals
            for json_cat in category_map.values():
                all_towers_totals_resolved[json_cat] += tower_totals_resolved[json_cat]
                all_towers_totals_open[json_cat] += tower_totals_open[json_cat]
            all_towers_total += tower_total
            
            row += 1
            
            # Write subcategory breakdowns
            for subcat in subcats:
                site_key = f"{tower} {subcat}"
                original_resolved_key = next((k for k, v in site_mapping.items() if v == ("tower", tower, subcat)), None)
                original_open_key = next((k for k, v in site_mapping.items() if v == ("tower", tower, subcat)), None)
                ...
                worksheet.write(row, 0, subcat, indented_site_format)
        ...
        # Write Total All Towers row
        worksheet.write(row, 0, "Total All Towers", total_all_format)
        for i, (display_cat, json_cat) in enumerate(category_map.items()):
            worksheet.write(row, i+1, all_towers_totals_resolved[json_cat], cell_format)
        for i, (display_cat, json_cat) in enumerate(category_map.items()):
            worksheet.write(row, i+4, all_towers_totals_open[json_cat], cell_format)
        worksheet.write(row, 7, all_towers_total, cell_format)
        ...
        def write_detail_sheet(sheet_name, data, title):
            ...
            # Write data
            row = 2
            site_mapping_details = {site: normalize_site_name(site) for site in data.keys()}
            for tower in towers.keys():
                for subcat in towers[tower]:
                    site_key = f"{tower} {subcat}"
                    original_key = next((k for k, v in site_mapping_details.items() if v == ("tower", tower, subcat)), None)
                    if original_key and original_key in data:
                        ...
                        for i in range(max_length):
                            detail_worksheet.write(row, 0, f"{tower} - {subcat}", site_format)
...
def generate_consolidated_ncr_Housekeeping_excel(combined_result, report_title="Housekeeping: Current Month"):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        date_part = f"{day}_{month_name}"  # "29_May"
        ...
        worksheet_summary.write(row, 1, f'No. of Housekeeping NCRs beyond 7 days ({report_type})', header_format)
        
        row = 2
        all_towers_total = 0
        
        # Handle tower-related sites
        for tower, subcats in towers.items():
            # Calculate total for this tower
            tower_total = 0
            for subcat in subcats:
                site_key = f"{tower} {subcat}"
                original_key = next((k for k, v in site_mapping.items() if v == ("tower", tower, subcat)), None)
                value = data[original_key].get("Count", 0) if original_key and original_key in data else 0
                tower_total += value
            
            # Write the tower name and its total
            worksheet_summary.write(row, 0, tower, site_format)
            worksheet_summary.write(row, 1, tower_total, cell_format)
            all_towers_total += tower_total
            row += 1
            ...
            for subcat in subcats:
                ...
                original_key = next((k for k, v in site_mapping.items() if v == ("tower", tower, subcat)), None)
                value = data[original_key].get("Count", 0) if original_key and original_key in data else 0
                worksheet_summary.write(row, 0, subcat, indented_site_format)
                worksheet_summary.write(row, 1, value, cell_format)
                row += 1
        
        # Write Total All Towers row
        worksheet_summary.write(row, 0, "Total All Towers", total_all_format)
        worksheet_summary.write(row, 1, all_towers_total, cell_format)
        row += 1
        ...
        for tower in towers.keys():
            for subcat in towers[tower]:
                ...
                if original_key and original_key in data:
                    ...
                    for i in range(max_length):
                        worksheet_details.write(row, 0, f"{tower} - {subcat}", site_format)
...
def generate_consolidated_ncr_Safety_excel(combined_result, report_title=None):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Update report_title with current date in "Month Day" format (e.g., "May 29")
        now = datetime.now()  # Current date: May 29, 2025
        day = now.strftime("%d")
        month_name = now.strftime("%B")
        ...
        # Define the hierarchical structure for tower-related sites
        towers = {
            "Tower 4": ["Pour 1", "Pour 2", "Common Description"],
            "Tower 5": ["Pour 1", "Pour 2", "Common Description"],
            "Tower 6": ["Pour 1", "Pour 2", "Common Description"],
        ...
        # Define non-tower sites
        non_tower_sites = ["Club", "Commercial", "External Development", "Common Area"]

        # Flat list for mapping purposes
        standard_sites = [f"{tower} {subcat}" for tower, subcats in towers.items() for subcat in subcats] + non_tower_sites

        def normalize_site_name(site):
            """
            Returns a tuple of (category, tower, subcategory) for tower sites,
            or (category, site) for non-tower sites.
            """
            # Check if the site is a tower-related site
            if site in standard_sites and site not in non_tower_sites:
                for tower, subcats in towers.items():
                    for subcat in subcats:
                        if site == f"{tower} {subcat}":
                            return "tower", tower, subcat
            ...
            if match:
                ...
                if "pour 1" in site.lower():
                    ...
                    return "tower", tower, "Common Description"
            # Check for non-tower sites
            if "club" in site.lower():
                return "non_tower", "Club", None
            if "commercial" in site.lower():
                return "non_tower", "Commercial", None
            if "external development" in site.lower():
                return "non_tower", "External Development", None
            return "non_tower", "Common Area", None
        ...
        # First, handle tower-related sites
        for tower, subcats in towers.items():
            # Calculate total for this tower
            tower_total = 0
            for subcat in subcats:
        ...
        # Then, handle non-tower sites
        for non_tower_site in non_tower_sites:
            original_key = next((k for k, v in site_mapping.items() if v == ("non_tower", non_tower_site, None)), None)
            value = data[original_key].get("Count", 0) if original_key and original_key in data else 0
            worksheet_summary.write(row, 0, non_tower_site, site_format)
            worksheet_summary.write(row, 1, value, cell_format)
            row += 1
        ...
        for non_tower_site in non_tower_sites:
            ...
            if original_key and original_key in data:
                ...
                max_length = max(len(descriptions), len(created_dates), len(close_dates), len(statuses))
                for i in range(max_length):
                    worksheet_details.write(row, 0, non_tower_site, site_format)
                    worksheet_details.write(row, 1, descriptions[i] if i < len(descriptions) else "", description_format)
                    worksheet_details.write(row, 2, created_dates[i] if i < len(created_dates) else "", cell_format)
                    worksheet_details.write(row, 3, close_dates[i] if i < len(close_dates) else "", cell_format)
                    worksheet_details.write(row, 4, statuses[i] if i < len(statuses) else "", cell_format)
...
def generate_combined_excel_report(all_reports, report_title=None):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        workbook = writer.book

        # Existing format definitions (unchanged)
        title_format = workbook.add_format({
            'bold': True,
            'align': 'center',
            'valign': 'vcenter',
        ...
        # Current date: May 29, 2025
        now = datetime.now()
        day = now.strftime("%d")
        month_name = now.strftime("%B")
        date_part = f"{day}_{month_name}"

        # Use the provided report_title if given, otherwise use the default format
        if report_title is None:
            date_part_title = f"{day}_{month_name}"
        else:
            date_part_title = report_title.replace("All_Reports_", "")  # Extract just the date part if needed

        # Define the hierarchical structure of sites (unchanged)
        towers = {
            "Tower 4": ["Pour 1", "Pour 2", "Common Description"],
            "Tower 5": ["Pour 1", "Pour 2", "Common Description"],
            "Tower 6": ["Pour 1", "Pour 2", "Common Description"],
        ...
        def normalize_site_name(site):
            ...
            if match:
                ...
                if "pour 1" in site.lower():
                    ...
                    return tower, "Common Description"
            return None, site

        # 1. NCR Report (Summary)
        combined_result = all_reports.get("Combined_NCR", {})
        report_title_ncr = f" Eden NCR: {date_part_title}"
        ...
        open_sites = open_data.get("Sites", {})
        site_mapping = {}
        for site in (resolved_sites.keys() | open_sites.keys()):
            tower, subcat = normalize_site_name(site)
            if tower:
                site_mapping[site] = (tower, subcat)
            else:
                site_mapping[site] = ("Unknown", site)
        ...
        for tower, subcats in towers.items():
            ...
            for subcat in subcats:
                site_key = f"{tower} {subcat}"
                original_resolved_key = next((k for k, v in site_mapping.items() if v == (tower, subcat)), None)
                original_open_key = next((k for k, v in site_mapping.items() if v == (tower, subcat)), None)
                site_total = 0
            ...
            # Write the tower name and its totals
            worksheet_ncr_summary.write(row, 0, tower, site_format)
            for i, (display_cat, json_cat) in enumerate(category_map.items()):
                worksheet_ncr_summary.write(row, i+1, tower_totals_resolved[json_cat], cell_format)
            for i, (display_cat, json_cat) in enumerate(category_map.items()):
                worksheet_ncr_summary.write(row, i+4, tower_totals_open[json_cat], cell_format)
            worksheet_ncr_summary.write(row, 7, tower_total, cell_format)
            ...
            for subcat in subcats:
                ...
                worksheet_ncr_summary.write(row, 0, subcat, indented_site_format)
        ...
        # Write Total All Towers row
        worksheet_ncr_summary.write(row, 0, "Total All Towers", total_all_format)
        for i, (display_cat, json_cat) in enumerate(category_map.items()):
            worksheet_ncr_summary.write(row, i+1, all_towers_totals_resolved[json_cat], cell_format)
        for i, (display_cat, json_cat) in enumerate(category_map.items()):
            worksheet_ncr_summary.write(row, i+4, all_towers_totals_open[json_cat], cell_format)
        worksheet_ncr_summary.write(row, 7, all_towers_total, cell_format)
        row += 1

        # 2. Closed NCR Details
        def write_detail_sheet(worksheet, data, title, date_part):
            ...
            for tower in towers.keys():
                for subcat in towers[tower]:
                    site_key = f"{tower} {subcat}"
                    original_key = next((k for k, v in site_mapping_details.items() if v == (tower, subcat)), None)
                    if original_key and original_key in data:
                        site_data = data[original_key]
                        descriptions = site_data.get("Descriptions", [])
                        created_dates = site_data.get("Created Date (WET)", [])
                        ...
                        max_length = max(len(descriptions), len(created_dates), len(close_dates), len(statuses), len(disciplines))
                        for i in range(max_length):
                            worksheet.write(row, 0, f"{tower} - {subcat}", site_format)
        ...
        if resolved_sites:
            worksheet_closed_details = workbook.add_worksheet(truncate_sheet_name(f'Closed NCR Details {date_part}'))
            write_detail_sheet(worksheet_closed_details, resolved_sites, "Closed NCR Details", date_part)

        # 3. Open NCR Details
        if open_sites:
            worksheet_open_details = workbook.add_worksheet(truncate_sheet_name(f'Open NCR Details {date_part}'))
            write_detail_sheet(worksheet_open_details, open_sites, "Open NCR Details", date_part)

        # 4. Safety NCR Summary (Combined Closed and Open)
        safety_closed_data = all_reports.get("Safety_NCR_Closed", {}).get("Safety", {}).get("Sites", {})
        safety_open_data = all_reports.get("Safety_NCR_Open", {}).get("Safety", {}).get("Sites", {})
        report_title_safety = f"Safety NCR: {day}_{month_name}"

        worksheet_safety_summary = workbook.add_worksheet(truncate_sheet_name(f'Safety NCR {date_part}'))
        worksheet_safety_summary.set_column('A:A', 20)
        worksheet_safety_summary.set_column('B:C', 15)

        worksheet_safety_summary.merge_range('A1:C1', report_title_safety, title_format)
        row = 1
        worksheet_safety_summary.write(row, 0, 'Site', header_format)
        worksheet_safety_summary.write(row, 1, 'Closed Safety NCRs beyond 7 days', header_format)
        worksheet_safety_summary.write(row, 2, 'Open Safety NCRs beyond 7 days', header_format)

        row = 2
        site_mapping_safety = {site: normalize_site_name(site) for site in (safety_closed_data.keys() | safety_open_data.keys())}
        all_towers_total_closed = 0
        all_towers_total_open = 0

        for tower, subcats in towers.items():
            # Calculate totals for this tower
            tower_total_closed = 0
            tower_total_open = 0
            
            for subcat in subcats:
                site_key = f"{tower} {subcat}"
                original_closed_key = next((k for k, v in site_mapping_safety.items() if v == (tower, subcat)), None)
                original_open_key = next((k for k, v in site_mapping_safety.items() if v == (tower, subcat)), None)

                closed_value = safety_closed_data[original_closed_key].get("Count", 0) if original_closed_key and original_closed_key in safety_closed_data else 0
                open_value = safety_open_data[original_open_key].get("Count", 0) if original_open_key and original_open_key in safety_open_data else 0
                
                tower_total_closed += closed_value
                tower_total_open += open_value
            
            # Write the tower name and its totals
            worksheet_safety_summary.write(row, 0, tower, site_format)
            worksheet_safety_summary.write(row, 1, tower_total_closed, cell_format)
            worksheet_safety_summary.write(row, 2, tower_total_open, cell_format)
            
            all_towers_total_closed += tower_total_closed
            all_towers_total_open += tower_total_open
            row += 1
            ...
            for subcat in subcats:
                ...
                worksheet_safety_summary.write(row, 0, subcat, indented_site_format)
                worksheet_safety_summary.write(row, 1, closed_value, cell_format)
                worksheet_safety_summary.write(row, 2, open_value, cell_format)
                row += 1

        # Write Total All Towers row
        worksheet_safety_summary.write(row, 0, "Total All Towers", total_all_format)
        worksheet_safety_summary.write(row, 1, all_towers_total_closed, cell_format)
        worksheet_safety_summary.write(row, 2, all_towers_total_open, cell_format)
        row += 1

        # 5. Housekeeping NCR Summary (Combined Closed and Open)
        housekeeping_closed_data = all_reports.get("Housekeeping_NCR_Closed", {}).get("Housekeeping", {}).get("Sites", {})
        housekeeping_open_data = all_reports.get("Housekeeping_NCR_Open", {}).get("Housekeeping", {}).get("Sites", {})
        report_title_housekeeping = f"Housekeeping NCR: {day}_{month_name}"

        worksheet_housekeeping_summary = workbook.add_worksheet(truncate_sheet_name(f'Housekeeping NCR {date_part}'))
        worksheet_housekeeping_summary.set_column('A:A', 20)
        worksheet_housekeeping_summary.set_column('B:C', 15)

        worksheet_housekeeping_summary.merge_range('A1:C1', report_title_housekeeping, title_format)
        row = 1
        worksheet_housekeeping_summary.write(row, 0, 'Site', header_format)
        worksheet_housekeeping_summary.write(row, 1, 'Closed Housekeeping NCRs beyond 7 days', header_format)
        worksheet_housekeeping_summary.write(row, 2, 'Open Housekeeping NCRs beyond 7 days', header_format)

        row = 2
        site_mapping_housekeeping = {site: normalize_site_name(site) for site in (housekeeping_closed_data.keys() | housekeeping_open_data.keys())}
        all_towers_total_closed = 0
        all_towers_total_open = 0
        ...
        for tower, subcats in towers.items():
            ...
            for subcat in subcats:
                site_key = f"{tower} {subcat}"
                original_closed_key = next((k for k, v in site_mapping_housekeeping.items() if v == (tower, subcat)), None)
                original_open_key = next((k for k, v in site_mapping_housekeeping.items() if v == (tower, subcat)), None)

                closed_value = housekeeping_closed_data[original_closed_key].get("Count", 0) if original_closed_key and original_closed_key in housekeeping_closed_data else 0
                open_value = housekeeping_open_data[original_open_key].get("Count", 0) if original_open_key and original_open_key in housekeeping_open_data else 0
                
                tower_total_closed += closed_value
                tower_total_open += open_value
            
            # Write the tower name and its totals
            worksheet_housekeeping_summary.write(row, 0, tower, site_format)
            worksheet_housekeeping_summary.write(row, 1, tower_total_closed, cell_format)
            worksheet_housekeeping_summary.write(row, 2, tower_total_open, cell_format)
            ...
            for subcat in subcats:
                ...
                worksheet_housekeeping_summary.write(row, 0, subcat, indented_site_format)
                worksheet_housekeeping_summary.write(row, 1, closed_value, cell_format)
                worksheet_housekeeping_summary.write(row, 2, open_value, cell_format)
                row += 1

        # Write Total All Towers row
        worksheet_housekeeping_summary.write(row, 0, "Total All Towers", total_all_format)
        worksheet_housekeeping_summary.write(row, 1, all_towers_total_closed, cell_format)
        worksheet_housekeeping_summary.write(row, 2, all_towers_total_open, cell_format)
        row += 1

        # 6. Combined Safety & Housekeeping Details
        worksheet_combined_details = workbook.add_worksheet(truncate_sheet_name(f'Safety & Housekeeping Details {date_part}'))
        worksheet_combined_details.set_column('A:A', 20)
        worksheet_combined_details.set_column('B:B', 60)
        worksheet_combined_details.set_column('C:D', 20)
        worksheet_combined_details.set_column('E:E', 15)
        worksheet_combined_details.set_column('F:F', 15)
        worksheet_combined_details.set_column('G:G', 15)

        worksheet_combined_details.merge_range('A1:G1', f"Safety & Housekeeping NCR Details {date_part}", title_format)
        headers = ['Site', 'Description', 'Created Date (WET)', 'Expected Close Date (WET)', 'Status', 'Discipline', 'Type']
        row = 1
        for col, header in enumerate(headers):
            worksheet_combined_details.write(row, col, header, header_format)

        row = 2
        for data, type_label in [
            (safety_closed_data, "Safety Closed"), (safety_open_data, "Safety Open"),
            (housekeeping_closed_data, "Housekeeping Closed"), (housekeeping_open_data, "Housekeeping Open")
        ]:
            site_mapping_details = {site: normalize_site_name(site) for site in data.keys()}
            for tower in towers.keys():
                for subcat in towers[tower]:
                    ...
                    if original_key and original_key in data:
                        ...
                        max_length = max(len(descriptions), len(created_dates), len(close_dates), len(statuses))
                        for i in range(max_length):
                            worksheet_combined_details.write(row, 0, f"{tower} - {subcat}", site_format)
                            worksheet_combined_details.write(row, 1, descriptions[i] if i < len(descriptions) else "", description_format)
                            worksheet_combined_details.write(row, 2, created_dates[i] if i < len(created_dates) else "", cell_format)
                            worksheet_combined_details.write(row, 3, close_dates[i] if i < len(close_dates) else "", cell_format)
                            worksheet_combined_details.write(row, 4, statuses[i] if i < len(statuses) else "", cell_format)
                            worksheet_combined_details.write(row, 5, "HSE", cell_format)
                            worksheet_combined_details.write(row, 6, type_label, cell_format)
                            row += 1
...
def generate_report_title(prefix, date_obj):
