...
project_name = st.sidebar.selectbox("Project Name", ["Wave Oakwood, Wave City", "WAVE CITY CLUB @ PSP 14A","EWS_LIG Veridia PH04", "GH-8 Phase-2 (ELIGO) Wave City","GH-8 Phase-3 (EDEN) Wave City"])
    ...
    if project_name == "GH-8 Phase-2 (ELIGO) Wave City":
        ...
        if st.session_state["ncr_df"] is not None and st.session_state["safety_df"] is not None and st.session_state["housekeeping_df"] is not None:
            ...
            if open_end is None:
                ...
                transformed_json = transform_json(combined_result_ncr)
                st.write(transformed_json)
                
                excel_open = generate_consolidated_ncr_Housekeeping_excel(open_result_housekeeping, f"Housekeeping: Open - {day}_{month_name}_{year}")
                st.download_button(
                    label="📥 Download Open Housekeeping Excel Report",
                    data=excel_open,
                    file_name=f"Housekeeping_NCR_Report_Open_{day}_{month_name}_{year}.xlsx",
                ...
                excel_closed = generate_consolidated_ncr_Safety_excel(closed_result_safety , report_title)
                st.download_button(
                    label="📥 Download Closed Safety Excel Report",
                    data=excel_closed,
                    file_name=f"Safety_NCR_Report_Closed_{day}_{month_name}_{year}.xlsx",
                ...
                excel_output = Excel_data(transformed_json)
                st.download_button(
                    label="📥 Download NCR open and close Report",
                    data=excel_output,
                    file_name="ncr_summary.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

                combined_excel_data = combine_excel_files(Excel_data(transformed_json), generate_consolidated_ncr_Safety_excel(closed_result_safety , report_title), generate_consolidated_ncr_Housekeeping_excel(open_result_housekeeping, f"Housekeeping: Open - {day}_{month_name}_{year}"))

                # Single download button
                st.download_button(
                    label="📥 Download Combined NCR Reports (All Data)",
                    data=combined_excel_data,
                    file_name=f"Combined_NCR_Reports_{day}_{month_name}_{year}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="download_combined_reports"
                )

                # combined_eligo = generate_consolidated_all_reports_excel_eligo(all_reports,transformed_json,"Open", "Close")
                # st.download_button(
        ...
        if st.session_state["ncr_df"] is not None and st.session_state["safety_df"] is not None and st.session_state["housekeeping_df"] is not None:
            ...
            if open_end is None:
                ...
                st.subheader("Combined NCR Report (JSON)")
                # ncr_open = combined_result_ncr["NCR open beyond 21 days"]
                # ncr_open["Site Names"] = ncr_open.pop("Sites")
                # combined_result_ncr["NCR open beyond 21 days"][st.session_state.project_name] = combined_result_ncr["NCR open beyond 21 days"].pop("Sites")
                ...
                # open_result_housekeeping['Housekeeping'][st.session_state.project_name] = open_result_housekeeping['Housekeeping'].pop("Sites")
